...
**Phase 3a (`dossier team create|join`): DELEGATING to agy (2026-07-16)**, base = current `team-sync` (`bee5507`). Spec against the REAL `core.Syncer`/`Service.Sync`/`config.Team`. `team create <url>` = init store as a git repo + origin + initial commit/push + write `team.remote`/`branch` to config; `team join <url>` = clone remote into store (shallow on first join per findings rec #6), write config, PAT setup. Thin CLI → `Service.TeamCreate`/`TeamJoin`. **Phase 3b (auto-sync on hooks) deferred** until 3a lands (per D6). Remaining Phase 4 (README Quickstart pointer, real-GitHub PAT dogfood, two-colleague pilot) not started.
