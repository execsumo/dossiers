...
Two delegations in flight (2026-07-15) — check their worktrees before starting overlapping work; if one died with no output, its scope below is self-contained enough to execute directly:
- **Phase 2 de-risk spike (go-git sync engine) → cline delegate**, worktree `dossiers-worktrees/cline-gitsync-spike`, branch `delegate/cline-gitsync-spike`. Scope-fenced to `internal/sync/**` + `go.mod`/`go.sum` + `docs/spikes/gitsync-findings.md` so it cannot collide with other work; it prototypes pull→resolve→commit→push against local bare repos, conflict detection on `dossier.md` (remote-wins + `ConflictRecord`, no merge markers), >100 MB exclusion, and the store-wide sync lock — NOT wired into `core.Service` (that wiring is Phase 2 proper).
- **Spec/docs alignment (plan Phase 4 doc items pulled forward) → antigravity delegate**, worktree `dossiers-worktrees/agy-teamsync-docs`, branch `delegate/agy-teamsync-docs`. Scope-fenced to `SPEC.md`, `PRD.md`, `PRFAQ.md`, `assets/guide.md` (+ guide golden fixtures if needed): PRD/PRFAQ single-user premise amendment, SPEC §7 `sync`/`team` command surface + §14 Team Sync acceptance criteria, guide.md multi-author etiquette note. Phase 2 *wiring* stays blocked on the spike's findings by design.
