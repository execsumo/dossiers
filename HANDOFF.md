...
**Phase 3 (onboarding commands + auto-sync): STARTING (2026-07-16).** Spec against the REAL `core.Syncer`/`Service.Sync` now on `team-sync`. Scope: `dossier team create|join`; sync piggybacks on Recall/Save/session hooks (async/debounced, never blocks); TUI/doctor/MCP surfacing. No daemon. Remaining Phase 4 (README Quickstart pointer, PRD/SPEC amendments, real-GitHub dogfood, two-colleague pilot) not started.
